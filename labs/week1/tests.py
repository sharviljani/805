...
def test_check_is_equal_to(results_dict):
    """
    Checks that lab1.check_is_equal_to(number1, number2) returns
    True if number1 is equal to number2, else False
    """
    print("")
    print("Testing lab1.check_is_equal_to()")
    message = "----"
    try:
        result = lab1.check_is_equal_to(2, 2)
        if not isinstance(result, bool):
            message += 'Did not return a boolean - Fail'
            test_failed(results_dict)
        else:
            message += 'A boolean was returned - Pass'
            test_passed(results_dict)
            # now check if the result is true (2 is equal to 4)
            if result is True:
                message += '\n---- 2 is equal to 2 , True - Pass'
                test_passed(results_dict)
            else:
                message += '\n---- 2 is equal to 2, False - Fail'
                test_failed(results_dict)
            # try again, with number1 = number2
            result = lab1.check_is_equal_to(6, 4)
            if result is False:
                message += '\n---- 6 is equal to 4, False - Pass'
                test_passed(results_dict)
            else:
                message += '\n---- 6 is equal to 4, True - Fail'
                test_failed(results_dict)
    except AttributeError:
        message += 'Could not find lab1.check_is_equal_to function - Fail'
        test_failed(results_dict)
    print(message)

def test_product_one_to_ten(results_dict):
    """
    Checks that lab1.product_one_to_ten() returns a the correct value
    """
    print("")
    print("Testing lab1.product_one_to_ten()")
    message = "----"
    try:
        result = lab1.product_one_to_ten()
        if not isinstance(result, int):
            message += 'Did not return an integer - Fail'
            test_failed(results_dict)
        else:
            ...
            message += 'An integer was returned - Pass'
            test_passed(results_dict)
            # now check if the value is correct, should be 3628800
            if result != ans:
                message += '\n----Did not return the correct number - Fail'
                test_failed(results_dict)
            else:
                message += '\n----The correct value (3628800) was returned - Pass'
                test_passed(results_dict)
    except AttributeError:
        message += 'Could not find lab1.product_one_to_ten function - Fail'
        test_failed(results_dict)
    print(message)
...
def run():
    ...
    test_check_is_equal_to(results_dict)
    test_product_one_to_ten(results_dict)
